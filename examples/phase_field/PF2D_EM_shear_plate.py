...
deg = 4
num_refinements = 6+1
output_filename = "pf2d_shear_plate_DEM"
...
def u_bound_dir_horiz_disp(x, y, disp=0.):
    return [disp, 0.0]

def u_bound_dir_phi_fixed(x, y):
    return [0.]

# # Set the boundary conditions
bound_down = boundary2D("Dirichlet", 0, "down", u_bound_dir_fixed)
bound_up = boundary2D("Dirichlet", 0, "up", u_bound_dir_horiz_disp)
bound_up_phi = boundary2D("Dirichlet", 0, "up", u_bound_dir_phi_fixed)
bound_cond_elast = [bound_down, bound_up]
bound_cond_phi = [bound_up_phi]
...
bcdof_u, _ = get_bcdof_bcval(meshes, bound_cond_elast)

model = PF2D_DEM_Spline(global_nodes_u, global_nodes_phi, deg, size_basis, bcdof_u,
...
for i_load in range(num_load_steps):
    ...
    bcdof_u, bcval_u = get_bcdof_bcval(meshes, bound_cond_elast)
    bcval_u = jnp.expand_dims(bcval_u, axis=-1)
    ...
    model.train(R, dR, jnp.squeeze(local_areas), bcval_u, fenerg, n_iter=50)
    
    t_start = time.time()
    params = model.get_params(model.opt_state)
    loss_func = jax_tfp_function_factory(model,  params, R, dR,
                                         jnp.squeeze(local_areas), bcval_u, fenerg)
    ...
    solver = jaxopt.LBFGS(model.get_loss_and_grads, value_and_grad=True,
                          maxiter=5000, tol=1e-14, jit = True)
    results, state = solver.run(params, R, dR, jnp.squeeze(local_areas), bcval_u, fenerg)

    print("Iteration: ", state[0].item(), " loss: ", state[1].item())
    _, unflatten_params = jax.flatten_util.ravel_pytree(params)  
  
    num_free_u = len(model.index_map) - len(bcdof_u)
    interior_vals_u = unflatten_params(results)[:num_free_u]
    interior_vals_phi = unflatten_params(results)[num_free_u:]
    print("Time taken (BFGS)", time.time()-t_start, "seconds")

    sol_u = jnp.zeros((2*size_basis))
    sol_u = sol_u.at[bcdof_u].set(bcval_u.flatten())
    ...
    top_disp += delta_disp
    u_bound_dir_var_horiz_disp = lambda x, y : [top_disp, 0.]
    # # Set the boundary conditions    
    bound_up = boundary2D("Dirichlet", 0, "up", u_bound_dir_var_horiz_disp)
    bound_cond_elast = [bound_down, bound_up]
