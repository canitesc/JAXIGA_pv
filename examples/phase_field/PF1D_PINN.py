...
Solve the equation l0^2*\phi''(x)- phi(x) = 0 with phi(0) = 1
...
class PF1D:
    def __init__(self, l0, rng_key, layers, activation=jax.nn.swish):
    ...
    def loss(self, params, inputs, targets, bnd_inputs, bnd_targets):
        ...
        u_pred = self.u(params, inputs)
        d2udx2_pred = self.d2u_vec(params, inputs)
        int_residual = self.l0**2*d2udx2_pred -  u_pred -  targets
...
model = PF1D(l0, key, [1, 32, 32, 32, 1])
params = model.get_params(model.opt_state)

# prepare the training data
x = jnp.linspace(xmin, xmax, num_pts)
x = jnp.expand_dims(x, 1)


exact_sol = lambda x : jnp.exp(-jnp.abs(x)/l0)
rhs_fun = lambda x : jnp.zeros_like(x)
f = rhs_fun(x)
x_bnd = jnp.array([xmin, 0., xmax])
x_bnd = jnp.expand_dims(x_bnd, 1)
u_bnd = exact_sol(x_bnd)

model.train(x, f, x_bnd, u_bnd, n_iter = 20000)
