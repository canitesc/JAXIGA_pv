...
Computes the inverse of the left-hand side matrix
...
from jaxiga.utils.Solvers import Lin_Inverse
...
#rhs_mat = jnp.identity(dim_space)
rhs = jnp.concatenate((jnp.array([[0.]]), fvals[1:-1], jnp.array([[0.]])), axis=0)

bcdof = jnp.array([0, dim_space-1])
bcval = jnp.array([[0.], [0.]])

rhs -= LHS[:, bcdof]@bcval
LHS = LHS[1:dim_space-1, 1:dim_space-1]
rhs = rhs[1:dim_space-1]
rhs_mat = jnp.identity(dim_space-2)

model = Lin_Inverse(dim_space)
model.train(LHS, rhs_mat)
t_start = time.time()
params = model.get_params(model.opt_state)
loss_func = jax_tfp_function_factory(model,  params, LHS, rhs_mat)
...
max_outer_iter = 20
num_outer_iter = 0
while current_loss > tolerance and num_outer_iter<max_outer_iter:
    results = bfgs_minimize(loss_func, initial_position = initial_pos,  max_iterations=1000)
...
LHS_inv = unflatten_params(results.position)
# LHS_inv = model.get_params(model.opt_state)
...
N_plot = eval_spline_basis(uhat_plot, p2e_plot.reshape(-1,1), C, deg)
