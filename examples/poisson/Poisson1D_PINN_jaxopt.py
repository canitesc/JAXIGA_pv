...
PINN optimization with JAXOPT -> seems slow and borked 25.05.2025
...
solver = jaxopt.BFGS(fun=loss_func,
                            value_and_grad=True,
                      maxiter=1000, tol=1e-6, linesearch='hager-zhang', 
                      verbose = False, jit = True)
params = loss_func.init_params_1d

params, state = solver.run(params)

print("Iteration: ", state[0].item(), " loss: ", state[1].item())
print("State =", state)
params = unflatten_params(params)
t2 = time.time()
