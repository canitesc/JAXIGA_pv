...
def bernstein_basis_3d(pts_u, pts_v, pts_w, deg):
    ...
    for k in range(deg[2] + 1):
        for j in range(deg[1] + 1):
            for i in range(deg[0] + 1):
                for kk in range(num_pts_w):
                    for jj in range(num_pts_v):
                        for ii in range(num_pts_u):
                            Buvw[ii, jj, kk, basis_counter] = B_u[ii, i] * B_v[jj, j] * B_w[kk, k]
                            dBdu[ii, jj, kk, basis_counter] = dB_u[ii, i] * B_v[jj, j] * B_w[kk, k]
                            dBdv[ii, jj, kk, basis_counter] = B_u[ii, i] * dB_v[jj, j] * B_w[kk, k]
                            dBdw[ii, jj, kk, basis_counter] = B_u[ii, i] * B_v[jj, j] * dB_w[kk, k]
                basis_counter += 1
