...
def find_vertex(vertices, vertex, tol_eq=1e-7):
    """
    ...
    for i in range(num_patches):
        ...
        for j in range(num_verts):
            ...
            if edge_key in edge_list:
                ...
                if (
                    indx_pt_a_patch_a > indx_pt_b_patch_a
                    and indx_pt_a_patch_b > indx_pt_b_patch_b
                ) or (
                    indx_pt_a_patch_a < indx_pt_b_patch_a
                    and indx_pt_a_patch_b < indx_pt_b_patch_b
                ):
...
def zip_conforming(mesh_list, vertex2patch, edge_list):
    ...
    corners = [
        0,
        mesh_list[0].deg[0],
        (mesh_list[0].deg[0] + 1) * (mesh_list[0].deg[1] + 1) - 1,
    ...
    for vert_index in range(len(vertex2patch)):
        for i in range(len(vertex2patch[vert_index])):
            ...
            element_index = get_element_from_corner(
                mesh_list[patch_index], corner_index
            )
            local_nodes = mesh_list[patch_index].elem_node[element_index][
    ...
    for patch_index in range(num_patches):
        unassigned_nodes = np.setdiff1d(
            range(mesh_list[patch_index].num_basis), assigned_nodes[patch_index]
        )
        ...
        for i_elem in range(mesh_list[patch_index].num_elem):
            mesh_list[patch_index].elem_node_global[i_elem] = nodes_pattern[
                patch_index
            ][mesh_list[patch_index].elem_node[i_elem]]
        # update the global boundary node indices
        mesh_list[patch_index].bcdof_global = {}
        for side in mesh_list[patch_index].bcdof:
            mesh_list[patch_index].bcdof_global[side] = nodes_pattern[patch_index][
                mesh_list[patch_index].bcdof[side]
            ].tolist()
