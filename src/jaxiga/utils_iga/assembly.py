...
from jaxiga.utils_iga.bernstein import bernstein_basis_2d
        ...
        for i_elem in range(mesh_list[i_patch].num_elem):
            ...
            for j_gauss in range(num_gauss_v):
                for i_gauss in range(num_gauss_u):
                    ...
                    local_stiff += (
                        local_area * a0(phys_pt[0], phys_pt[1]) * (dR.transpose() @ dR)
                    )
                    domain_area += local_area

            II[index_counter : index_counter + num_nodes ** 2] = np.tile(
                global_nodes, num_nodes
            )
            JJ[index_counter : index_counter + num_nodes ** 2] = np.repeat(
                global_nodes, num_nodes
            )
            S[index_counter : index_counter + num_nodes ** 2] = np.reshape(
                local_stiff, num_nodes ** 2
            )
            index_counter += num_nodes ** 2
        ...
        for i_elem in range(mesh_list[i_patch].num_elem):
            ...
            for j_gauss in range(num_gauss_v):
                for i_gauss in range(num_gauss_u):
                    ...
                    local_mass += (
                        local_area * a1(phys_pt[0], phys_pt[1]) * np.outer(RR, RR)
                    )
                    domain_area += local_area
...
def stiff_elast_2D(mesh_list, Cmat, gauss_rule):
    ...
    for i_patch in range(len(mesh_list)):
        for i_elem in range(mesh_list[i_patch].num_elem):
            ...
            II[index_counter : index_counter + 4 * num_nodes ** 2] = np.tile(
                global_nodes_xy, 2 * num_nodes
            )
            JJ[index_counter : index_counter + 4 * num_nodes ** 2] = np.repeat(
                global_nodes_xy, 2 * num_nodes
            )
            S[index_counter : index_counter + 4 * num_nodes ** 2] = np.reshape(
                local_stiff, 4 * num_nodes ** 2
            )
            index_counter += 4 * num_nodes ** 2
