...
from jaxiga.utils.bernstein import bernstein_basis_2d
...
class boundary1D:
    ...
    side : (string) side of the parameters space (e.g. "down", "right")
...
def applyBC1D(mesh_list, bound_cond, lhs, rhs):
    """
    ...
    for i in range(len(bound_cond)):
        ...
        if bound_cond[i].type == "Dirichlet":
            ...
            assert 0, "Boundary type not implemented"
...
def applyNeumannElast2D(mesh, rhs, side, quad_rule_side, op_val):
    ...
    for i_elem in mesh.elem[side]:
        ...
        global_nodes_xy = np.reshape(
            np.stack((2 * global_nodes[sctr], 2 * global_nodes[sctr] + 1), axis=1),
            2 * len(sctr),
        )
        cpts = mesh.cpts[0:2, local_nodes]
        wgts = mesh.wgts[local_nodes]
...
def get_bcdof_bcval(mesh_list, bound_cond):
    ...
    for i in range(len(bound_cond)):
        ...
        if bound_cond[i].type == "Dirichlet":
            # check x-direction
            bound_val = bound_cond[i].op_value(eval_pt[0], eval_pt[1])
            if bound_val[0] != None:
...
def applyBCElast2D(mesh_list, bound_cond, lhs, rhs, quad_rule):
    ...
    return lhs, rhs#, bcdof, bcval


def applyBCElast_DEM_2D(mesh_list, bound_cond, size_basis, quad_rule, index_map):
    """
    Applies the Neumann boundary conditions to a linear system for 2D elasticity
