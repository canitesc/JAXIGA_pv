...
from jaxiga.utils.splines import bspdegelev, bspkntins

class Geometry1D:
    """
    Base class for 1D geometry
    """
    def __init__(self, geomData):
        self.curve = NURBS.Curve()
        self.curve.degree = geomData["degree_u"]
        # self.curve.ctrlpts_size = geomData["ctrlpts_size_u"]
        self.curve.ctrlpts = geomData["ctrlpts"]
        self.curve.weights = geomData["weights"]
        self.curve.knotvector = geomData["knotvector_u"]
        
    def getCoefs(self):
        """
        Outputs the control points in the NURBS toolbox format as an
        nd-array of size 4 x nu  where the 4 rows contain
        ...
        """
        coefs_ctrl = np.array(self.curve.ctrlpts).transpose()
        coefs_ctrl = np.reshape(
            coefs_ctrl, (3, -1)
        )
        coefs_weights = np.array(self.curve.weights)
    ...
    def setCoefs(self, coefs):
        ...
        """
        num_u = coefs.shape[1]        
        self.curve.ctrlpts_size_u = num_u        
        PctrlPts = np.zeros((num_u, 3))
        weights = np.zeros(num_u)
        ctrl_pt_counter = 0
        for j in range(num_u):            
                PctrlPts[ctrl_pt_counter, :] = coefs[0:3, j] / coefs[3, j]
                weights[ctrl_pt_counter] = coefs[3, j]
                ctrl_pt_counter += 1
        self.curve.ctrlpts = PctrlPts.tolist()
        self.curve.weights = weights.tolist()
        
    def mapPoints(self, uPar):
        """
        Map points from the parameter domain [0,1] to the physical domain
        ...
        gpParamU = uPar.tolist()        
        res = np.array(self.curve.evaluate_list(gpParamU))
    ...
    def getUnifIntPts(self, numPtsU, withEdges):
        ...
        Input: numPtsU - number of points (including edges) in the u direction
                         in the parameter space
               withEdges - 1x2 array of zeros or ones specifying whether the boundary points
                           should be included. The boundary order is [left, right]
        Output: xPhys - flattened array containing the x coordinates of the points
        """
    ...
    def knot_insert(self, ins_u_knots):
        """
        ...
        """
        coefs = self.getCoefs()
        knots_u = self.curve.knotvector
        
        # Insert the knots along the u direction
        if len(ins_u_knots) > 0:
            coefs, knots_u = bspkntins(self.curve.degree, coefs, knots_u, ins_u_knots)            

        self.curve.reset(evalpts=True, ctrlpts=True)
        self.setCoefs(coefs)
        self.curve.knotvector = knots_u
        
    def refine_knotvectors(self, u_refine):
        """
        ...
        tol_eq = 1e-10
        if u_refine:
            for i in range(len(self.curve.knotvector) - 1):
                if (
                    abs(self.curve.knotvector[i + 1] - self.curve.knotvector[i])
                    > tol_eq
                ):
                    new_knot = (
                        self.curve.knotvector[i + 1] + self.curve.knotvector[i]
                    ) / 2.0
                    new_knots_u.append(new_knot)    

        self.knot_insert(new_knots_u)
        
    def degreeElev(self, utimes):
        """
        ...
        if utimes != 0:            
            coefs, knots_u = bspdegelev(self.curve.degree, coefs, knots_u, utimes)

        self.curve.reset(evalpts=True, ctrlpts=True)
        self.curve.degree += utimes        
...
class Geometry2D:
    ...
    def __init__(self, geomData):
        ...
        if 'ctrlpts2d' in geomData:
            self.surf.ctrlpts2d = geomData['ctrlpts2d']
        else:
...
class Geometry3D:
    '''
     ...
     Keys: degree_u, degree_v, degree_w: polynomial degree in the u, v, w directions
       ...
       ctrlpts: weighted control points (in a list with 
           ...
           The control points should vary in the u-direction first, then v-direction
           then w-direction
    ...
    def __init__(self, geomData):
        ...
        self.vol.ctrlpts = geomData['ctrlpts']
    ...
    def getCoefs(self):
        """
        Outputs the control points in the NURBS toolbox format as an
        nd-array of size 4 x nu x nv x nw where the 4 rows contain
        the (weigthed) control points and weights, nu is the number of control
        points in the u direction, nv is the number of control points in the v
        direction, and nw is the number of control points in the w direction

        Output: coefs - the control points and weights in NURBS toolbox format
        """
        coefs_ctrl = np.array(self.vol.ctrlpts).transpose()
        coefs_ctrl = np.reshape(
            coefs_ctrl, (3, self.vol.ctrlpts_size_u, self.vol.ctrlpts_size_v,
                         self.vol.ctrlpts_size_w), order='F'
        )
        #coefs_ctrl = np.transpose(coefs_ctrl, (0,2,1,3))
        coefs_weights = np.array(self.vol.weights)
        coefs_weights = np.reshape(
            coefs_weights, (1, self.vol.ctrlpts_size_u, self.vol.ctrlpts_size_v, 
                            self.vol.ctrlpts_size_w), order='F'
        )
        #coefs_weights = np.transpose(coefs_weights, (0,2,1,3))
        coefs_ctrl_weighted = coefs_ctrl * coefs_weights
        coefs = np.concatenate((coefs_ctrl_weighted, coefs_weights))
        return coefs
    ...
    def setCoefs(self, coefs):
        ...
        num_v = coefs.shape[2]
        num_w = coefs.shape[3]
        self.vol.ctrlpts_size_u = num_u
        self.vol.ctrlpts_size_v = num_v
        self.vol.ctrlpts_size_w = num_w
        PctrlPts = np.zeros((num_u * num_v * num_w, 3))
        weights = np.zeros(num_u * num_v * num_w)
        ctrl_pt_counter = 0
        for i in range(num_w):
            for k in range(num_v):
                for j in range(num_u):
                    PctrlPts[ctrl_pt_counter, :] = coefs[0:3, j, k, i] / coefs[3, j, k, i]
                    weights[ctrl_pt_counter] = coefs[3, j, k, i]
                    ctrl_pt_counter += 1
        self.vol.ctrlpts = PctrlPts.tolist()
        self.vol.weights = weights.tolist()
    ...
    def getDerivatives(self, vertex, uCoord, vCoord, wCoord, elmtNo):
        '''
    ...
    def getQuadFacePts(self, numElem, numGauss, orient):
        ...
        Input: numElem - 1x3 array with the number of number of subdivisions
               numGauss - number of Gauss points per subdivision (in each direction)
               orient - face orientation in parameter space: 1 is front (v=0), 
        ...
        #vertex = self.genElemList(numElem[0], numElem[1], numElem[2])
        elemInfo = self.computeC()
        vertex = elemInfo['vertex']
        self.C = elemInfo['C']
        self.nodes = elemInfo['nodes']
        ...
        for i1E in range(numElem[1]):                
            for i0E in range(numElem[0]):
                ...
                for iPt in range(numGauss**2):
                    ...
                    elmtNo = self.findspan(curPtU, curPtV, curPtW)                        
                    physPtX, physPtY, physPtZ, jacMat = self.getDerivatives(vertex, curPtU, curPtV, curPtW, elmtNo)
    ...
    def degreeElev(self, utimes, vtimes, wtimes):
        """
        Degree elevate the 2D geometry from degree (p,q,r) to degree
        (p+utimes, q+vtimes, r+wtimes)

        Parameters
        ----------
        utimes : difference in polynomial degree in u direction
        vtimes : difference in polynomial degree in v direction
        wtimes : difference in polynomial degree in w direction
        ...
        num_v = coefs.shape[2]
        num_w = coefs.shape[3]
        knots_u = self.vol.knotvector_u
        knots_v = self.vol.knotvector_v
        knots_w = self.vol.knotvector_w
        if wtimes != 0:
            coefs = np.reshape(coefs, (4*num_u*num_v,num_w))
            coefs, knots_w = bspdegelev(self.vol.degree_w, coefs, knots_w, wtimes)
            num_w = coefs.shape[1]
            coefs = np.reshape(coefs, (4, num_u, num_v, num_w))                    
        
        if vtimes != 0:
            coefs = np.transpose(coefs, (0, 1, 3, 2))
            coefs = np.reshape(coefs, (4 * num_u*num_w, num_v))
            coefs, knots_v = bspdegelev(self.vol.degree_v, coefs, knots_v, vtimes)
            num_v = coefs.shape[1]
            coefs = np.reshape(coefs, (4, num_u, num_w, num_v))
            coefs = np.transpose(coefs, (0, 1, 3, 2))
        ...
        if utimes != 0:
            ...
            coefs = np.reshape(coefs, (4 * num_v * num_w, num_u))
            coefs, knots_u = bspdegelev(self.vol.degree_u, coefs, knots_u, utimes)
            num_u = coefs.shape[1]
            coefs = np.reshape(coefs, (4, num_v, num_w, num_u))
            coefs = np.transpose(coefs, (0, 3, 1, 2))

        self.vol.reset(evalpts=True, ctrlpts=True)
        self.vol.degree_u += utimes
        self.vol.degree_v += vtimes
        self.vol.degree_w += wtimes

        self.setCoefs(coefs)
        self.vol.knotvector_u = knots_u
        self.vol.knotvector_v = knots_v
        self.vol.knotvector_w = knots_w
        
    def knot_insert(self, ins_u_knots, ins_v_knots, ins_w_knots):
        """
        Insert single or multiple knots
        ...
        ins_u_knots : (list) knots to be inserted in the u direction
        ins_v_knots : (list) knots to be inserted in the v direction
        ins_w_knots : (list) knots to be inserted in the w direction
        """
        coefs = self.getCoefs()
        num_u = coefs.shape[1]
        num_v = coefs.shape[2]
        ...
        # Insert the knots along the w direction
        if len(ins_w_knots) > 0:
            coefs = np.reshape(coefs, (4*num_u*num_v, num_w))
            coefs, knots_w = bspkntins(self.vol.degree_w, coefs, knots_w, ins_w_knots)
            num_w = coefs.shape[1]
            coefs = np.reshape(coefs, (4, num_u, num_v, num_w))                    
        ...
        if len(ins_v_knots) > 0:
            coefs = np.transpose(coefs, (0, 1, 3, 2))
            coefs = np.reshape(coefs, (4 * num_u*num_w, num_v))
            coefs, knots_v = bspkntins(self.vol.degree_v, coefs, knots_v, ins_v_knots)
            num_v = coefs.shape[1]
            coefs = np.reshape(coefs, (4, num_u, num_w, num_v))
            coefs = np.transpose(coefs, (0, 1, 3, 2))
        ...
        if len(ins_u_knots) > 0:
            coefs = np.transpose(coefs, (0, 2, 3, 1))
            coefs = np.reshape(coefs, (4 * num_v*num_w, num_u))
            coefs, knots_u = bspkntins(self.vol.degree_u, coefs, knots_u, ins_u_knots)
            num_u = coefs.shape[1]
            coefs = np.reshape(coefs, (4, num_v, num_w, num_u))
            coefs = np.transpose(coefs, (0, 3, 1, 2))
    ...
    def refine_knotvectors(self, u_refine, v_refine, w_refine):
        """
        Refine the geometry by uniform knot insertion
        ...
        v_refine : boolean
            ...
            of non-empty knotspans
        w_refine : boolean
            If true, refine the w knotvector by inserting knots at the midpoint
            of non-empty knotspans
        ...
        tol_eq = 1e-10
        if u_refine:
            for i in range(len(self.vol.knotvector_u) - 1):
                if (
                    abs(self.vol.knotvector_u[i + 1] - self.vol.knotvector_u[i])
                    > tol_eq
                ):
                    new_knot = (
                        self.vol.knotvector_u[i + 1] + self.vol.knotvector_u[i]
                    ) / 2.0
                    new_knots_u.append(new_knot)
        
        if v_refine:
            for i in range(len(self.vol.knotvector_v) - 1):
                if (
                    abs(self.vol.knotvector_v[i + 1] - self.vol.knotvector_v[i])
                    > tol_eq
                ):
                    new_knot = (
                        self.vol.knotvector_v[i + 1] + self.vol.knotvector_v[i]
                    ) / 2.0
                    new_knots_v.append(new_knot)
        
        if w_refine:
            for i in range(len(self.vol.knotvector_w) - 1):
                if (
                    abs(self.vol.knotvector_w[i + 1] - self.vol.knotvector_w[i])
                    > tol_eq
                ):
                    new_knot = (
                        self.vol.knotvector_w[i + 1] + self.vol.knotvector_w[i]
                    ) / 2.0
                    new_knots_w.append(new_knot)
                    
        self.knot_insert(new_knots_u, new_knots_v, new_knots_w)
