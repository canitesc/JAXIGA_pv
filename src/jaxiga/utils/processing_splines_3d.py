...
def make_all_cpts_wgts_fem_3d(num_patches, num_elems, deg, elem_nodes, global_nodes,
                              cpts, wgts, Cs, num_fields):
    num_elem = sum(num_elems)
    num_nodes = (deg+1)**3
    cpts_all = jnp.zeros((num_elem, num_nodes, 3))
    wgts_all = jnp.zeros((num_elem, num_nodes))
    global_nodes_all = jnp.zeros((num_elem, num_fields*(deg+1)**3), dtype=int)
    C_all = jnp.stack(Cs)
    elem_indx = 0
    for i_patch in range(num_patches):
        local_nodes = elem_nodes[i_patch]
        cpts_all = cpts_all.at[elem_indx:elem_indx+num_elems[i_patch]].set(cpts[i_patch][0:3,
...
def pde_form_poisson_3d(R, dR, phys_pt, local_area, param_funs, aux_fields):
    a0 = param_funs[0]
    f = param_funs[1]
    local_stiff = local_area * \
        a0(phys_pt[0], phys_pt[1], phys_pt[2]) * (dR.transpose() @ dR)
    local_rhs = local_area * f(phys_pt[0], phys_pt[1], phys_pt[2]) * R
    return local_stiff, jnp.squeeze(local_rhs)


def pde_form_noop_2d(R, dR, phys_pt, local_area, param_funs, aux_fields):
    num_nodes = R.shape[1]
    local_stiff = jnp.zeros((num_nodes, num_nodes))
    local_rhs = jnp.zeros_like(R)
    return local_stiff, local_rhs


def pde_form_elast_3d(R, dR, phys_pt, local_area, params, aux_fields):
    num_nodes = R.shape[1]
    Cmat = params[0].Cmat
    B = jnp.zeros((3 * num_nodes, 6))
    B = B.at[0: 3 * num_nodes - 2: 3, 0].set(dR[0, :])
    B = B.at[1: 3 * num_nodes - 1: 3, 1].set(dR[1, :])
    B = B.at[2: 3 * num_nodes: 3, 2].set(dR[2, :])
    
    B = B.at[0: 3 * num_nodes - 2: 3, 3].set(dR[1, :])
    B = B.at[1: 3 * num_nodes - 1: 3, 3].set(dR[0, :])
    
    B = B.at[1: 3 * num_nodes - 1: 3, 4].set(dR[2, :])
    B = B.at[2: 3 * num_nodes: 3, 4].set(dR[1, :])
    
    B = B.at[0: 3 * num_nodes - 2: 3, 5].set(dR[2, :])
    B = B.at[2: 3 * num_nodes: 3, 5].set(dR[0, :])

    local_stiff = local_area * (B @ Cmat @ B.transpose())
    local_rhs = jnp.zeros(3*num_nodes)
    return local_stiff, local_rhs


def pde_form_elast_pf_2d(R, dR, phys_pt, local_area, params, aux_fields):
    num_nodes = R.shape[1]
    Cmat = params[0].Cmat
    phi = aux_fields[0]
...
def pde_form_phi_pf_2d(R, dR, phys_pt, local_area, params, aux_fields):
    cenerg = params[0]
    l = params[1]
    fenerg = aux_fields[0]
    local_stiff = local_area * (cenerg * l * dR.transpose()@dR + (cenerg/l + 2*fenerg)*R.transpose()@R)
    local_rhs = local_area * 2 * R * fenerg
    return local_stiff, jnp.squeeze(local_rhs)


def field_form_scalar_3d(R, dR, local_sol_vals, params):
    local_field = jnp.dot(R, local_sol_vals)
    return local_field

def field_form_scalar_deriv_3d(R, dR, local_sol_vals, params):
    local_field = jnp.dot(dR, local_sol_vals)
    return local_field

def field_form_strains_3d(R, dR, local_sol_vals, params):
    num_nodes = R.shape[1]
    B = jnp.zeros((3 * num_nodes, 6))
    B = B.at[0: 3 * num_nodes - 2: 3, 0].set(dR[0, :])
    B = B.at[1: 3 * num_nodes - 1: 3, 1].set(dR[1, :])
    ...
    B = B.at[0: 3 * num_nodes - 2: 3, 5].set(dR[2, :])
    B = B.at[2: 3 * num_nodes: 3, 5].set(dR[0, :])
    local_field = jnp.dot(local_sol_vals, B)
    return local_field

def _compute_basis_3d(ijk_gauss, carry, C, Buvw, dBdu, dBdv, dBdw, u_diff, v_diff,
                      w_diff, cpts, wgts):
    i_gauss = ijk_gauss[0]
    j_gauss = ijk_gauss[1]
    k_gauss = ijk_gauss[2]
    ...
    # Bezier extraction
    N_mat = C @ Buvw[i_gauss, j_gauss, k_gauss, :]
    dN_du = C @ dBdu[i_gauss, j_gauss, k_gauss, :] * 2 / u_diff
    dN_dv = C @ dBdv[i_gauss, j_gauss, k_gauss, :] * 2 / v_diff
    dN_dw = C @ dBdw[i_gauss, j_gauss, k_gauss, :] * 2 / w_diff
    ...
    dw_xi = jnp.sum(dRdu)
    dw_eta = jnp.sum(dRdv)
    dw_zeta = jnp.sum(dRdw)

    dRdu = dRdu / w_sum - R * dw_xi / w_sum ** 2
    dRdv = dRdv / w_sum - R * dw_eta / w_sum ** 2
    dRdw = dRdw / w_sum - R * dw_zeta / w_sum ** 2

    # compute the solution w.r.t. the physical space
    dR = jnp.stack((dRdu, dRdv, dRdw))
    dxdxi = dR @ cpts
    ...
    return R, dR, jac_par_phys


def _compute_local_stiffs_3d(pde_form, param_funs, carry, x):
    ...
    local_stiff_temp, local_rhs_temp = pde_form(
        R, dR, phys_pt, local_area, param_funs, aux_fields)
    local_stiff += local_stiff_temp
    local_rhs += local_rhs_temp
    elem_area += local_area
    return ([local_stiff, local_rhs, elem_area], None)

def _compute_local_basis_3d(jac_ref_par, cpts, wgts, u_diff, v_diff, w_diff, C,
                             Buvw, dBdu, dBdv, dBdw, wgts_u, wgts_v, wgts_w, carry,
                             ijk_gauss):
    i_gauss = ijk_gauss[0]
    j_gauss = ijk_gauss[1]
    k_gauss = ijk_gauss[2]

    RR, dR, jac_par_phys = _compute_basis_3d(ijk_gauss, carry, C, Buvw,
                                             dBdu, dBdv, dBdw, u_diff, v_diff,
                                             w_diff, cpts, wgts)
    local_area = (
        jac_par_phys * jac_ref_par * wgts_u[i_gauss] * wgts_v[j_gauss] * wgts_w[k_gauss]
    )
    phys_pt = RR @ cpts
    # print("phys_pt = ", phys_pt)
    # plt.scatter(phys_pt[0], phys_pt[1], s=0.5, c="black")
    local_R = jnp.reshape(RR, (1, -1))
    local_dR = jnp.reshape(dR, (3, -1))
    local_areas = jnp.array([[local_area]])
    
    return (None, (local_R, local_dR, local_areas, phys_pt))

def _compute_local_field_3d(field_form, param_funs, sol_vals, carry, x):
    R = x[0]
    dR = x[1]
    local_field = field_form(R, dR, sol_vals, param_funs)
    return (None, local_field)


@partial(jax.jit, static_argnums=(7, 8, 9))
def spline_basis_fun_3d(jac_ref_par, cpts, wgts, u_diff, v_diff, w_diff, C,
                       num_gauss_u, num_gauss_v, num_gauss_w, Buvw, dBdu, dBdv,
                       dBdw, wgts_u, wgts_v, wgts_w):
    ...
    for k_gauss in range(num_gauss_w):
        for j_gauss in range(num_gauss_v):
            for i_gauss in range(num_gauss_u):
                ijk_gausses.append([i_gauss, j_gauss, k_gauss])
    carry = None

    _compute_local_partial = partial(_compute_local_basis_3d, jac_ref_par,
                                     cpts, wgts, u_diff, v_diff, w_diff, C,
                                     Buvw, dBdu, dBdv, dBdw, wgts_u, wgts_v, wgts_w)

    carry, ys = lax.scan(_compute_local_partial,
                              carry, jnp.array(ijk_gausses))
    local_R = ys[0]
    local_dR = ys[1]
    local_areas = ys[2]
    phys_pt = ys[3]
    return local_R, local_dR, local_areas, phys_pt

@partial(jax.jit, static_argnums=(4, 5, 6, 7))
def local_stiff_fun_3d(R, dR, local_areas, phys_pts, num_fields, num_nodes,
                       pde_form, param_funs, aux_fields):
    local_stiff = jnp.zeros((num_fields*num_nodes, num_fields*num_nodes))
    local_rhs = jnp.zeros(num_fields*num_nodes)
    elem_volume = jnp.array([[0.]])

    carry = [local_stiff, local_rhs, elem_volume]
    xs = [R, dR, local_areas, phys_pts, aux_fields]

    _compute_local_partial = partial(_compute_local_stiffs_3d, pde_form,
                                     param_funs)

    carry, _ = lax.scan(_compute_local_partial, carry, xs)
    local_stiff = carry[0]
    local_rhs = carry[1]
    elem_volume = carry[2]
    return local_stiff, local_rhs, elem_volume


@partial(jax.jit, static_argnums=(3, 4, 5))
def local_field_3d(R, dR, sol_vals, num_fields, field_form, params):
    # compute the rational spline basis
    carry = None
    xs = [R, dR]
    _compute_local_partial = partial(_compute_local_field_3d, field_form, params, sol_vals)
    carry, field_pt = lax.scan(_compute_local_partial, carry, xs)
...
def make_global_nodes_xyz(global_nodes, num_fields):
    global_nodes_xyz = []
    for global_node in global_nodes:
        global_node_xyz = num_fields*global_node.reshape(-1,1)
        for i in range(num_fields-1):
            global_node_xyz = jnp.concatenate(
                (global_node_xyz, num_fields*global_node.reshape(-1,1)+i+1), axis=1)
        global_nodes_xyz.append(jnp.reshape(
            global_node_xyz, num_fields*len(global_node)))

    return global_nodes_xyz
...
def extract_mesh_variables(mesh_list, num_fields):
    ...
    for i_patch in range(num_patches):
        ...
        global_nodes_xyz = make_global_nodes_xyz(mesh_list[i_patch].elem_node_global,
                                               num_fields)
        global_nodes.append(global_nodes_xyz)
        num_elems.append(mesh_list[i_patch].num_elem)
        u_mins = mesh_list[i_patch].elem_vertex[:, 0]
        u_maxs = mesh_list[i_patch].elem_vertex[:, 3]
        v_mins = mesh_list[i_patch].elem_vertex[:, 1]
        v_maxs = mesh_list[i_patch].elem_vertex[:, 4]
        w_mins = mesh_list[i_patch].elem_vertex[:, 2]
        w_maxs = mesh_list[i_patch].elem_vertex[:, 5]
        u_diff = u_maxs - u_mins
        v_diff = v_maxs - v_mins
        w_diff = w_maxs - w_mins

        jac_ref_par = u_diff * v_diff * w_diff / 8
        ...
        w_diffs = jnp.concatenate((w_diffs, w_diff))
        for i_elem in range(mesh_list[i_patch].num_elem):
            Cs.append(mesh_list[i_patch].C[i_elem])
    return (cpts, wgts, elem_nodes, global_nodes, num_elems, Cs, u_diffs, v_diffs,
            w_diffs, jac_ref_pars)


@timing
def evaluate_spline_basis_fem_3d(mesh_list, gauss_rule, num_fields):
    t = time.time()
    deg = mesh_list[0].deg
    ...
    Buvw, dBdu, dBdv, dBdw = bernstein_basis_3d(pts_u, pts_v, pts_w, deg)
    elapsed = time.time() - t
    print("Evaluating 3D Bezier polynomials took", elapsed, "seconds")

    t = time.time()
    # Allocate memory for the triplet arrays
    (cpts, wgts, elem_nodes, global_nodes,
     num_elems, Cs, u_diffs, v_diffs, w_diffs, 
                 jac_ref_pars) = extract_mesh_variables(mesh_list, num_fields)

    elapsed = time.time() - t
    print("Extracting the mesh variables took ", elapsed, " seconds")
    num_patches = len(mesh_list)
    t = time.time()
    cpts_all, wgts_all, C_all, global_nodes_all = make_all_cpts_wgts_fem_3d(num_patches,
                                                                            tuple(
                                                                                num_elems), deg[0],
    ...
    t = time.time()
    R, dR, local_areas, phys_pts = jax.vmap(spline_basis_fun_3d,
                                     in_axes=(0, 0, 0, 0, 0, 0, 0,
                                             ...
                                             None))(jac_ref_pars,
                                                    ...
                                                    wgts_v,
                                                    wgts_w)
    return R, dR, local_areas, phys_pts, global_nodes_all

def evaluate_stiff_rhs_fem_3d(R, dR, local_areas, phys_pts, gauss_rule, num_fields,
                              global_nodes_all, pde_form, param_funs, aux_fields):
    num_nodes = R.shape[3]
    t = time.time()
    aux_ax = len(aux_fields)*(0,)

    local_stiffs, local_rhss, elem_volumes = jax.vmap(local_stiff_fun_3d,
                                        in_axes=(0, 0, 0, 0,  None,
                                               None, None, None, aux_ax))(R,
                                                                        dR,
                                                                        local_areas,
                                                                        phys_pts,                                                                      
                                                                        num_fields,
                                                                        ...
                                                                        aux_fields)                          
    elapsed = time.time() - t
    print("Evaluating the local stiffness matrices took", elapsed, " seconds")
    print("Sum of elem volumes is", jnp.sum(elem_volumes))
    # make the triplet arrays
    t = time.time()
    II, JJ, S = jax.vmap(make_triplet_array)(global_nodes_all, local_stiffs)
    II = jnp.reshape(II, -1)
    JJ = jnp.reshape(JJ, -1)
    S = jnp.reshape(S, -1)
    ...
    return II, JJ, S, local_rhss

def evaluate_field_fem_3d(R, dR, num_fields, global_nodes_all, 
                          field_form, sol_val, params):
    sol_vals_all = sol_val[global_nodes_all]
    #t = time.time()
    field_vals = jax.vmap(local_field_3d,
                          in_axes=(0, 0, 0, None, None, None))(R,
                                                                dR,
                                                                sol_vals_all,                                                
                                                                num_fields,
                                                                field_form,
