...
def eval_spline_basis(u_hat, p2e, C, deg):
    ...
    for i in range(len(u_hat)):
        new_row = jnp.dot(C[p2e[i][0]], bernstein_basis(u_hat[i], deg).T).T
...
def pde_form_poisson_2d(R, dR, phys_pt, local_area, param_funs, aux_fields):
    a0 = param_funs[0]
    f = param_funs[1]
...
def _compute_local_stiffs_2d(pde_form, param_funs, carry, x):
...
def _compute_local_basis_2d(jac_ref_par, cpts, wgts, u_diff, v_diff, C,
                             Buv, dBdu, dBdv, wgts_u, wgts_v, carry, ij_gauss):
    i_gauss = ij_gauss[0]
    j_gauss = ij_gauss[1]
...
def spline_basis_fun_2d(jac_ref_par, cpts, wgts, u_diff, v_diff, C,
    ...
    _compute_local_partial = partial(_compute_local_basis_2d, jac_ref_par,
                                     cpts, wgts, u_diff, v_diff, C,
                                     Buv, dBdu, dBdv, wgts_u, wgts_v)
...
def local_stiff_fun_2d(R, dR, local_areas, phys_pts, num_fields, num_nodes,
    ...
    _compute_local_partial = partial(_compute_local_stiffs_2d, pde_form,
                                     param_funs)
...
@timing
def evaluate_spline_basis_fem_2d(mesh_list, gauss_rule, num_fields):
    t = time.time()
    deg = mesh_list[0].deg
    ...
    t = time.time()
    R, dR, local_areas, phys_pts = jax.vmap(spline_basis_fun_2d,
                                     in_axes=(0, 0, 0, 0, 0, 0,
                                             None, None, None,
                                             None, None, None,
                                             None))(jac_ref_pars,
...
def evaluate_stiff_rhs_fem_2d(R, dR, local_areas, phys_pts, gauss_rule, num_fields,
                              global_nodes_all, pde_form, param_funs, aux_fields):
    num_nodes = R.shape[3]
    t = time.time()
    aux_ax = len(aux_fields)*(0,)

    local_stiffs, local_rhss, elem_areas = jax.vmap(local_stiff_fun_2d,
