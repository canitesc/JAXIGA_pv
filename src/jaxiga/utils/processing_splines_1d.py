...
def make_all_cpts_wgts_fem_1d(num_patches, num_elems, deg, elem_nodes, global_nodes,
                              cpts, wgts, Cs, num_fields):
    num_elem = sum(num_elems)
    num_nodes = deg+1
    cpts_all = jnp.zeros((num_elem, num_nodes, 1))
    wgts_all = jnp.zeros((num_elem, num_nodes))
    global_nodes_all = jnp.zeros((num_elem, num_fields*(deg+1)), dtype=int)
    C_all = jnp.stack(Cs)
    elem_indx = 0
    for i_patch in range(num_patches):
        local_nodes = elem_nodes[i_patch]
        cpts_all = cpts_all.at[elem_indx:elem_indx+num_elems[i_patch]].set(cpts[i_patch][0:1,
...
def pde_form_poisson_1d(R, dR, phys_pt, local_area, param_funs, aux_fields):
    a0 = param_funs[0]
    f = param_funs[1]
    local_stiff = local_area * \
        a0(phys_pt[0]) * (dR.transpose() @ dR)
    local_rhs = local_area * f(phys_pt[0]) * R
    return local_stiff, jnp.squeeze(local_rhs)

def pde_form_mass_1d(R, dR, phys_pt, local_area, param_funs, aux_fields):
    a0 = param_funs[0]
    f = param_funs[1]
    local_stiff = local_area * \
        a0(phys_pt[0]) * (R.transpose() @ R)
    local_rhs = local_area * f(phys_pt[0]) * R
    return local_stiff, jnp.squeeze(local_rhs)
...
def pde_form_elast_2d(R, dR, phys_pt, local_area, params, aux_fields):
    num_nodes = R.shape[1]
    Cmat = params[0].Cmat
...
def pde_form_elast_2d_comp(R, dR, phys_pt, local_area, params, aux_fields):
    num_nodes = R.shape[1]
    nu = params[0]
    Emod = aux_fields[0]
    #jax.debug.print('Emod.shape = {}', Emod.shape)
    #jax.debug.print('nu = {}', nu)
    Cmat = Emod / (1 - nu ** 2) * jnp.array([[1, nu, 0],
                                             [nu, 1, 0],
                                             [0, 0, (1 - nu) / 2]])
    #jax.debug.print('Cmat = {}', Cmat)
    ...
    local_stiff = local_area * (B @ Cmat @ B.transpose())
    #jax.debug.print("local_stiff = {}", local_stiff)
    local_rhs = jnp.zeros(2*num_nodes)
    return local_stiff, local_rhs
...
def field_form_scalar_2d(R, dR, local_sol_vals, params):
    local_field = jnp.dot(R, local_sol_vals)
    return local_field

def field_form_scalar_deriv_2d(R, dR, local_sol_vals, params):
    local_field = jnp.dot(dR, local_sol_vals)
    return local_field

def field_form_strains_2d(R, dR, local_sol_vals, params):
    num_nodes = R.shape[1]
    B = jnp.zeros((2 * num_nodes, 3))
    B = B.at[0: 2 * num_nodes - 1: 2, 0].set(dR[0, :])
    B = B.at[1: 2 * num_nodes: 2, 1].set(dR[1, :])
...
def _compute_basis_1d(i_gauss, carry, C, Bu, dBdu, u_diff, cpts, wgts):    
    # compute the B-spline basis functions and derivatives with
    # Bezier extraction
    N_mat = C @ Bu[i_gauss[0], :]
    dN_du = C @ dBdu[i_gauss[0], :] * 2 / u_diff    
...
def _compute_local_stiffs_1d(pde_form, param_funs, carry, x):
...
def _compute_local_basis_1d(jac_ref_par, cpts, wgts, u_diff, C,
                             Bu, dBdu, wgts_u, carry, i_gauss):    

    RR, dR, jac_par_phys = _compute_basis_1d(i_gauss, carry, C, Bu,
                                             dBdu, u_diff, cpts, wgts)
    local_area = (
    ...
    local_dR = jnp.reshape(dR, (1, -1))
    local_areas = jnp.array([local_area])
    
    return (None, (local_R, local_dR, local_areas, phys_pt))

def _compute_local_field_2d(field_form, param_funs, sol_vals, carry, x):
    R = x[0]
    dR = x[1]
...
def spline_basis_fun_1d(jac_ref_par, cpts, wgts, u_diff, C,
                       num_gauss_u, Bu, dBdu, wgts_u):
    # compute the rational spline basis
    i_gausses = []

    for i_gauss in range(num_gauss_u):
        i_gausses.append([i_gauss])
    carry = None

    _compute_local_partial = partial(_compute_local_basis_1d, jac_ref_par,
                                     cpts, wgts, u_diff, C, Bu, dBdu, wgts_u)

    carry, ys = lax.scan(_compute_local_partial,
                              carry, jnp.array(i_gausses))
    local_R = ys[0]
    local_dR = ys[1]
    local_areas = ys[2]
    phys_pt = ys[3]
...
def local_stiff_fun_1d(R, dR, local_areas, phys_pts, num_fields, num_nodes,
    ...
    elem_area = jnp.array([[0.]])

    carry = [local_stiff, local_rhs, elem_area]
    xs = [R, dR, local_areas, phys_pts, aux_fields]

    _compute_local_partial = partial(_compute_local_stiffs_1d, pde_form,
                                     param_funs)
    ...
    local_rhs = carry[1]
    elem_area = carry[2]
    return local_stiff, local_rhs, elem_area


@partial(jax.jit, static_argnums=(3, 4, 5))
def local_field_2d(R, dR, sol_vals, num_fields, field_form, params):
    # compute the rational spline basis
    carry = None
    xs = [R, dR]
    _compute_local_partial = partial(_compute_local_field_2d, field_form, params, sol_vals)
...
def extract_mesh_variables(mesh_list, num_fields):
    ...
    for i_patch in range(num_patches):
        ...
        u_maxs = mesh_list[i_patch].elem_vertex[:, 1]        
        u_diff = u_maxs - u_mins        

        jac_ref_par = u_diff / 2
    ...
    return cpts, wgts, elem_nodes, global_nodes, num_elems, Cs, u_diffs, jac_ref_pars


@timing
def evaluate_spline_basis_fem_1d(mesh_list, gauss_rule, num_fields):
    t = time.time()
    deg = mesh_list[0].deg
    pts_u = gauss_rule[0]["nodes"]    
    wgts_u = gauss_rule[0]["weights"]    
    num_gauss_u = len(pts_u)   
    Bu, dBdu = bernstein_basis_1d(pts_u, deg)
    elapsed = time.time() - t
    print("Evaluating 1D Bezier polynomials took", elapsed, "seconds")

    t = time.time()
    # Allocate memory for the triplet arrays
    (cpts, wgts, elem_nodes, global_nodes,
     num_elems, Cs, u_diffs, jac_ref_pars) = extract_mesh_variables(mesh_list,
                                                                    num_fields)
    ...
    num_patches = len(mesh_list)
    t = time.time()
    cpts_all, wgts_all, C_all, global_nodes_all = make_all_cpts_wgts_fem_1d(num_patches,
                                                                            tuple(
                                                                                num_elems), deg,
    ...
    t = time.time()
    R, dR, local_areas, phys_pts = jax.vmap(spline_basis_fun_1d,
                                     in_axes=(0, 0, 0, 0, 0,
                                             None, None, None,
                                             None))(jac_ref_pars,
                                                    cpts_all,
...
def evaluate_stiff_rhs_fem_1d(R, dR, local_areas, phys_pts, gauss_rule, num_fields,
                              global_nodes_all, pde_form, param_funs, aux_fields):
    num_nodes = R.shape[3]
    t = time.time()
    aux_ax = len(aux_fields)*(0,)

    local_stiffs, local_rhss, elem_areas = jax.vmap(local_stiff_fun_1d,
    ...
    print("Sum of elem areas is", jnp.sum(elem_areas))
...
def evaluate_field_fem_2d(R, dR, num_fields, global_nodes_all, 
